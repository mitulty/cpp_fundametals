/**
 * @Author: Mitul Tyagi
 * @Date:   2025-02-04 20:38:54
 * @Description: List in C++
 */
/*
- Lists are sequence containers that allow non-contiguous memory allocation. As compared to the vector, the list has slow traversal, but once a position has been found, insertion and deletion are
  quick (constant time).
...
- C++ List is a STL container that stores elements randomly in unrelated locations. To maintain sequential ordering, every list element includes two links: one that points to the previous element and
  another that points to the next element.
- In C++, the STL list implements the doubly-linked list data structure. This allow iteration in both forward and backward direction. It provides only sequential access. Random Access to any middle
...
int main(int argc, char const *argv[])
{
    ...
    cout << "-> Deleting element from begin to 2:  ";
