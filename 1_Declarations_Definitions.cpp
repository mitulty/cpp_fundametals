...
- Some definitions explicitly specify a value for the entities they define. For types, aliase, templates, functions and constants, the value
  is permanent. For non-const data types, the initial value may be changed later.
...
- Except for function and namespace definitions, a declaration is terminated by a semicolon. A specifier is an initial keyword that specifies some non-type
  attribute of what is being declared. A declarator is composed of a name and optionally some declarator operators.
- Declarator Operators:
        1. Prefix:*(pointer), *const (constant pointer), *volatile(volatile pointer), & (lvalue reference), &&(rvalue reference) and auto.
        2. Postfix: [] (array), () (function) and  -> (returns from function).
- It is possible to declare several names in a single declaration. The declaration simply contains a list of comma-separated declarations. Operations apply
  to individual names only and not to any subsequent names in the same declaration.
...
- Nonlocal names starting with an underscore are reserved for special facilities in the implementation and the run-time environment and should not be used.
  Similarly, names starting with a double underscore (__) or an underscore followed by an uppercase letter are reserved.
- Uppercase and lowercase letters are distinct.
- When reading a program, the compiler always looks for the longest string of characters that could make up a name.
- Variable is a name (identifier) given to a memory location. It is the basic unit of storage in a program.
- A declaration introduces a name into a scope; that is, a name can be used only in a specific part of the program text:
    ...
    4. Global Scope: A name is called a global name if it is defined outside any function, lambda, class, enum class, or a namespace. The scope of a global
                     name extends from the point of declaration to the end of the file in which its declaration occurs.
    5. Statement Scope: A name is in a statement scope if it is defined within the () part of a for, while, if, or switch statement. All names in statement
                        scope are local names.
    6. Function Scope: A label is in scope from its point of declaration until the end of the function.
- A declaration of a name in a block can hide a declaration in an enclosing block or a global name. That is, a name can be redefined to refer to a different
  entity with in a block. After exit from the block, the name resumes its previous meaning.
- A hidden global name can be referred to using the scope resolution operator, :: . There is no way to use a hidden local name.
- The scope of a name that is not a class member starts at its point of declaration, that is, after the complete declarator and before the initializer. This
  implies that a name can be used to specify its own initial value. It is possible to use a single name to refer to two different objects in a block without
  using the :: operator.
- The names of function arguments and lambda arguments are considered declared in the outermost block of a function.
- A declaration is not allowed as the only statement on the branch of an if-statement.
- If an initializer is specified for an object, that initialiazer determines the initial value of an object. An initializer can use one of four syntactic
  styles:
        X a1{v}; X a2 = {v}; X a3 = v; X a4(v);
- Initialization using {}, list initialization, does not allow narrowing. That is an integer can not be converted to another integer that cannot holds it value
  , a floating-point value can not be converted to another floating-point type that cannot hold its value, a floating-point value cannot be converted to an
  integer type and an integer value can not be coverted to a floating-point type.
- When using auto prefer = .
- The empty initializer list, {}, is used to determine that a default value is desired. Most types have a default value. For integral types, the default
  value is a suitable representation of zero. For pointers, the default value is nullptr. For user-defined types, the default value (if any) is defined by the
  type's constructors.
- For user-defined types, there can be a distinction between direct initialization (where implicit conversions are allowed) and copy initialization (where
  they are not).
- If no initializer is specified, a global, namespace, local static, or static member is initialized to {} of the appropriate type. Local variables and objects
  created on the free store (sometimes called dynamic objects or heap objects) are not intialized by default unless they are of user-defined types with a
  default constructor.
- To initialize local variables of built-in types or objects of built-in type created with new, use {}.
- A member of an array or a structure is default initialized if the array or structure is.
- To use more than one value as an intializer, initializer lists delimited by { and } are used. In some cases, function-style argument lists can also be used.
  In a declaration, an empty pair of paranthesis, (), always means function.
- When using auto, a {}-list has its type deduced to std::initializer_list<T>. auto is a placeholder for the type of the initializer.
- Braced initialization of a variable declared with a placeholder type but without `=` requires exactly one element inside the braces. For deduction to
...
- The language provides two mechanisms for deducing a type from an expression:
      1. auto for deducing a type of an object from its initializer; the type can be the type of a variable, a const, or a constexpr.
      2. decltype(expr) for deducing the type of something that is not a simple initializer, such as the return type for a function or the type of a class
         member.
- When a decalartion of a variable has an initializer, then the auto can be used as a placeholder for the type of the initializer. The type of a homogeneous
  list of elements of type T is taken to be of type initializer_list<T>.
- When using auto, there is only one type involved, the type of the intializer, and = can be used safely.
...
- Initialization Terminology:
      ...
      -> Default Initialization: Object is initialized only if corresponding constructor is defined.
      -> Zero Initialization: Object initialized by (convereted) 0. Used for global/static/thread-local objects.
      -> Value Initialization: Object always gets a value (initialized by constructor or zero initializer).
      -> List Initialization: Object is initialized by braces( both direct list-initialized or copy list-initialized). Object always gets a value (initialized
                              by constructor or zero initialized).
      -> Aggregate Initialization: Special form of list initialization if type is aggregate.
- Variable Lifetime: The period of time in which a variable is alive in the memory. It becomes alive when declared and it is killed from memory at some point.
- Storage Duration: It defines how long the variable will be alive in the memory. These are: local(block, automatic); static; dynamic.
...
const char *season[] = {"Spring", "Summer", "Fall", "Winter"}; // declaration and definition
...
void f3(double val, int val2)
{
  ...
  // char c5{264}; // error: a=narrowing conversion of '264' from 'int' to 'char'
  ...
  vector<string> vec1{"Hello!!"}; // vec1 is a vector of 1 element with the value "Hello!"
  // vector<string> vec2("Hello!!"); // error: no vector constructor takes a string literal
  int x4{};                  // x4 becomes 0
  ...
  // = is redundant for all the below declaration,defintion and intialization using {}
  ...
  vector<double> v = {0.0,
                      1.1,
                      2.2,
                      3.3}; // use list constructor

  complex<double> a(0, pi * 10); // use constructor
  vector<int> v_int(10, 3);      // use constructor: v_int gets 10 elements intialized to 3
  complex<double> fz();          // function declaration
  complex<double> fz1(1, 2);     // function-style inititializer(initialization by constructor)
  complex<double> fz_1{};        // initialization by constructor to the default value{0,0}
  complex<double> fz1_1{1, 2};   // initialization by constructor to {1,2}
  auto m1 = {1, 2, 3, 4};        // m1 is an initializer_list<int>. Copy list initialization
  auto m2 = {1.0, 2.3, 4.56};    // m2 is an initializer_list<double>. Copy list initialization

  // auto z01 {1,3,4) //This will not work. For deduction to ‘std::initializer_list’, use copy-list-initialization (i.e. add ‘=’ before the ‘{’)
...
// extern float error_numner; // This will not work as redeclaration is not of the same type.
int error_number = 5;
// int count = 3; // This is a redefintion of count and hence will give compilation error.
/*
 In func()
Second Local x: 2
Global x: 7
