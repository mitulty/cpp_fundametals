/**
 * @Author: Mitul Tyagi
 * @Date:   2025-02-04 20:33:55
 * @Description: Vectors in C++
 */
/*
- In C++, vectors are used to store elements of similar data types. However, unlike arrays, the size of a vector can grow dynamically. They are used by
  including vector header file.
- The vector resize() changes the size of the vector (i.e., number of elements) while the vector reserve() changes the capacity (amount of memory allocated)
  of the vector. Reducing the size with vector resize() does not release unused memory. The capacity remains the same. Reserve method can not decrease the
  capacity and does not change the size too.
- In C++, vector shrink_to_fit() is a built-in function used to reduce the capacity of the vector to fit its size and destroys all elements beyond the size.
 */
#include <iostream>
#include <bits/stdc++.h>
using namespace std;
...
int main(int argc, char const *argv[])
{
    ...
    // The at() function is preferred zover [] because at() throws an exception whenever the vector is out of bound, while [] gives a garbage value.
    ...
    vector<int> vector_iter = {132, 562, 2353, 467, 5960, 3466, 7897, 1248, 99345};
    ...
    for (vector<int>::iterator it = vector_iter.begin(); it != vector_iter.end(); it++)
        cout << *(it) << " ";
    cout << endl;
    ...
    int *ptr = v.data();
    cout << "-> Accessing using a pointer to the first element of a vector: " << *vector_iter.data() << endl;
